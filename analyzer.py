...
def get_predefined_reports(selected_years: list):
    ...
    try:
        for name, category in report_queries.items():
            query = (session.query(Document.company_name, Document.report_year, Finding.finding_text, Document.filename).join(Finding).filter(Finding.category == category).filter(Document.report_year.in_(selected_years))); reports[name] = pd.read_sql(query.statement, session.bind)
    finally: session.close()
    return reports
...
def get_new_findings(year_to_check, base_year):
    ...
    session = Session()
    try:
        query_new = (session.query(Document.company_name, Finding.finding_text, Finding.category).join(Finding).filter(Document.report_year == year_to_check)); df_new = pd.read_sql(query_new.statement, session.bind)
        query_base = (session.query(Document.company_name, Finding.finding_text).join(Finding).filter(Document.report_year == base_year)); df_base = pd.read_sql(query_base.statement, session.bind)
...
def main():
    ...
    if not Session_factory: st.stop()
