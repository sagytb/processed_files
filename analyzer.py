# analyzer.py
# FINAL ULTIMATE HYBRID VERSION: Fixed the deprecated 'experimental_rerun' command.
    ...
    def setup_database():
        if IS_CLOUD and not os.path.exists(LOCAL_DB_PATH):
            ...
            try:
                ...
                progress_bar.empty(); info_message.success("הורדת בסיס הנתונים הושלמה!"); time.sleep(2); info_message.empty(); st.cache_data.clear()
            except requests.exceptions.RequestException as e:
                info_message.error(f"שגיאה בהורדת בסיס הנתונים: {e}"); return None
        if not os.path.exists(LOCAL_DB_PATH):
            st.error(f"קובץ בסיס הנתונים '{LOCAL_DB_PATH}' לא נמצא."); return None
        engine = db.create_engine(f'sqlite:///{LOCAL_DB_PATH}'); Base = declarative_base()
...
if not st.session_state.startup_complete:
    # This block will run once at the start.
    if DEEPSEEK_API_KEY:
        Session = setup_database()
        if Session:
            st.session_state.Session = Session
            st.session_state.startup_complete = True
            st.rerun() # Use the correct, modern rerun command
    else:
        st.error("מפתח API של DeepSeek לא הוגדר. אנא הגדר אותו והפעל מחדש.")
        st.stop()
else:
    # If startup is complete, run the main app UI with the established session
